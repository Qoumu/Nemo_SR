...
def build_prototypical_dataset(
    ...
    *,
    ...
    print(f"  Valid: min={min(val_counts.values())}, max={max(val_counts.values())}, avg={sum(val_counts.values())/len(val_counts):.1f}")
    print(f"  Test: min={min(test_counts.values())}, max={max(test_counts.values())}, avg={sum(test_counts.values())/len(test_counts):.1f}")
